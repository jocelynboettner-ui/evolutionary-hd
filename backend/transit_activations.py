...
def degree_to_gate(longitude: float) -> int:
    """Convert ecliptic longitude (0-360°) to HD gate number."""
    lon = longitude % 360
...
def get_transit_gates_today(date: Optional[datetime] = None) -> dict:
    ...
    for name, planet_id in planets.items():
        try:
            ...
            result[name] = degree_to_gate(pos[0])
        except Exception:
            try:
...
def find_activated_channels(
    ...
    A channel lights up when one gate is natal AND the partner gate
    is activated by a transiting planet.
    """
    activated = []
...
def find_temporarily_defined_centers(
    ...
    """
    Find undefined centers that become temporarily defined
    when transit gates complete channels into them.
    """
    natal_set = set(natal_gates)
    transit_gate_set = {g for g in transit_gates.values() if g}
    ...
    for center, gates in CENTER_GATES.items():
        ...
        center_channels = [
            (g1, g2, name) for (g1, g2), (c1, c2, name) in CHANNELS.items()
...
def get_cycle_incarnation_cross(
    ...
    """
    Calculate the dominant Incarnation Cross active during a cycle window
    by finding the most frequently activated cross pattern across the period.
    Uses Sun position (primary cross activator) sampled monthly.
    """
    ...
    if not cross_counts:
        return {"cross": "Unable to calculate", "frequency": 0}
...
def get_full_transit_activation(
    ...
    cycle_cross = get_cycle_incarnation_cross(cycle_start, cycle_end, natal_gates)
