...
function parseBirthData(text) {
    ...
    if (isoMatch) {
          ...
          if (m1) {
                  month = MONTHS[m1[1].substring(0,3).toLowerCase()]; day = m1[2].padStart(2,"0"); year = m1[3];
          } else if (m2) {
                  day = m2[1].padStart(2,"0"); month = MONTHS[m2[2].substring(0,3).toLowerCase()]; year = m2[3];
          }
    }
...
export default function App() {
    ...
    async function sendMessage() {
          ...
          // This ensures a new person in the same session gets a clean slate
          let currentBirthdata = birthdata;
          let isNewPerson = false;
      
          const parsed = parseBirthData(text);
          if (parsed) {
                  // New birth data detected — always treat as new person
                  // even if birthdata was already set from a previous person
                  isNewPerson = true;
                  currentBirthdata = parsed;
                  setBirthdata(parsed);
                  setChartDetected(true);
          } else if (!currentBirthdata) {
                  // No birth data in this message and none stored
                  isNewPerson = false;
          }
      
          // If this is a new person's birth data, start with a clean message history
          // so old chart data doesn't ride along to the server
          const newMessages = isNewPerson ? [userMsg] : [...messages, userMsg];
            ...
            {/* Header */}
                <div style={{
                  ...
                  {chartDetected && birthdata && (
                      <div style={{ marginTop: "10px", fontSize: "12px", color: "rgba(134,239,172,0.85)", background: "rgba(134,239,172,0.08)", border: "1px solid rgba(134,239,172,0.2)", borderRadius: "20px", padding: "4px 16px", display: "inline-block" }}>
